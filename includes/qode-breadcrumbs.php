		...
		if(is_product() ||bridge_qode_is_product_category()) {
			...
			// 	echo bridge_qode_get_module_part($before . get_the_time('Y') . $after);
				...
				if ( isset( $taxonomies[$taxonomy] ) ) {
					$taxonomy_name = '<a href="' . home_url('/' . $taxonomy . '/') . '">' . $taxonomies[$taxonomy]->labels->name . '</a>';
				}
				$term_obj = get_term_by( 'slug', $term, $taxonomy, 'OBJECT' );
				if( !empty( $term_obj ) ) {
					$term_name = $term_obj->name;
				}
			  	echo bridge_qode_get_module_part($before . $taxonomy_name . $delimiter . $term_name . $after);
			} elseif (is_author()) {
				global $author;
