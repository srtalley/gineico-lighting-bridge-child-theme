...
$raq_order_statuses = wc_get_order_statuses();
if(isset($raq_order_statuses['wc-ywraq-new'])) {
	unset($raq_order_statuses['wc-ywraq-new']);
}
$raq_order_statuses = array_keys($raq_order_statuses);
$YITH_YWRAQ_order_statuses = YITH_YWRAQ_Order_Request()->raq_order_status;
if(($key = array_search('wc-ywraq-new', $YITH_YWRAQ_order_statuses)) !== false) {
    unset($YITH_YWRAQ_order_statuses[$key]);
}
$customer_quotes = get_posts( apply_filters( 'ywraq_my_account_my_quotes_query', array(
    ...
    'post_status' => array_merge($YITH_YWRAQ_order_statuses, $raq_order_statuses)
) ) );
?>
...
<?php if ( $customer_quotes ) : ?>
	...
	<?php if( get_option( 'ywraq_show_reject_link' ) != 'no' ): ?>
	    ...
	    <div id="reject-quote-modal" class="hide-modal">
			<p> <?php _e( 'Are you sure that you want to reject the quote n.', 'yith-woocommerce-request-a-quote' )  ?>  <span id="modal-order-number"></span>?</p>
	        <p><a class="ywraq-button button reject-quote-modal-button" href="#" data-order_id=""><?php echo apply_filters( 'ywraq_reject_quote_button_text', __('Yes, I want to reject the quote', 'yith-woocommerce-request-a-quote') ); ?></a>
